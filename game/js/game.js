...
const progressStore = createProgressStore(userId, levelIds);

const settingsStore = createSettingsStore(userId);
...
async function startGame() {
  ...
  try {
    ...
    const manifestLevel = levels.find(
      (level) => level.id === Number(levelData.id),
    );

    const state = progressStore.getSnapshot();

    /*
      Do not allow direct URL access to an unfinished locked
      level.
    */
    ...
    /*
      This records that the player started another attempt.
    */
